...
function BasketPage() {
    ...
    const handleChekout = async ()=>{
        ...
        try {
            const metadata: Metadata = {
                ...
                orderName: user?.fullName ?? "Unknown",
                customerEmail: user?.emailAddresses[0].emailAddress ?? "Unknown", 
                clerkUserId: user!.id,
            }

            const checkoutUrl = await createChekoutSession(groupItems,metadata);
    ...
    return (
        ...
        >
            <h1 className='text-2xl font-bold mb-4'>You Basket</h1>
                    ...
                    {groupItems?.map((item) => (
                        <div
                            ...
                            <div className="flex items-center cursor-pointer flex-1 min-w-0"
                                onClick={() => {
                                    route.push(`/product/${item.product.slug?.current}`)
                                }}
                            >
